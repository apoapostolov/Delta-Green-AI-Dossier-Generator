...
const PortraitSelector: React.FC<{ isSelected: boolean; onClick: (e: React.MouseEvent) => void }> = ({ isSelected, onClick }) => (
    <button
        onClick={onClick}
        className="absolute top-2 left-2 z-30 w-8 h-8 bg-black/60 rounded-full flex items-center justify-center border-2 border-gray-500 hover:border-green-400 transition-all"
                ...
                <div className="absolute inset-2">
                     ...
                     {showDownload && (
                        <button
                            ...
                            className="absolute top-2 right-2 z-20 p-2 bg-black/50 rounded-full text-white hover:bg-green-500/80 hover:text-gray-900 transition-all duration-200"
