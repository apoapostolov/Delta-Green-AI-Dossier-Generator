...
export const SourcesModal: React.FC<SourcesModalProps> = ({ onClose }) => {
    ...
    const handleToggleSource = (sourceId: SourceID) => {
        ...
        if (!newSelection.has('delta-green')) {
            const hasOtherSelection = Array.from(newSelection).some(id => id !== 'delta-green');
            if (!hasOtherSelection) {
                // This case happens when only 'delta-green' is selected and the user tries to toggle it off.
                return; 
            }
        }
