...
const COMPLEX_PROFESSION_SEEDS: ComplexProfessionSeed[] = [
  ...
  seed('Marine Interdiction', 'Federal Agent', 'CBP Air and Marine Operations', 'Boarding specialists who close with suspicious vessels from smaller craft and conduct interdiction actions at sea.', ['Alertness', 'Athletics', 'Search', 'SIGINT'], 'Federal-agent equipment plus water-survival gear and boarding tools.', ['The OCR includes a bespoke marine-interdiction skill package focused on Spanish, boat handling, HUMINT, and close-quarters boarding work.'], {
    bonds: 2,
  }),
  ...
  seed('Tactical Operations', 'Special Operator', 'ATF Special Response Teams', 'High-risk ATF tactical operators built for violent warrant service, manhunts, and close support to investigations where initiative decides survival.', ['Alertness', 'Demolitions', 'Firearms', 'Search'], 'Tactical-entry gear, breaching tools, and standard SRT assault equipment.', ['The OCR around this entry is heavily interleaved with neighboring profiles, but the role is clearly the ATF SRT assault operator track.']),
  ...
  seed('Personal Protective Detail Division', 'Federal Agent', 'U.S. Secret Service', 'Bodyguards and site-control specialists assigned to the most visible and politically sensitive VIP protection missions in the U.S. government.', ['Athletics', 'First Aid', 'Military Science (Land)', 'Search'], 'Federal-agent equipment, low-visibility protective gear, and the logistics package needed for advance work and site control.', ['The OCR provides a partial custom skill package blending investigation, bodyguard work, mobility, and suppression.'], {
    bonds: 1,
  }),
  ...
  seed('Search and Rescue Team', 'Firefighter', 'U.S. Coast Guard', 'Rescuers who confront brutal seas, bad weather, and failing machinery head-on to save lives at the edge of survivability.', ['Alertness', 'First Aid', 'Navigate', 'Swim'], 'Portable rescue gear, thermal and water-survival suits, and specialist maritime kits.', ['The OCR includes a custom SAR profile built around repair skills, meteorology, navigation, boats, helicopters, and lifesaving in terrible conditions.'], {
    bonds: 2,
  }),
  ...
  seed('Interagency Threat Assessment and Coordination Group', 'Intelligence Analyst', 'National Counterterrorism Center', 'Interagency analysts who try to move critical threat data across bureaucratic seams that rarely cooperate as smoothly as policy says they should.', ['Bureaucracy', 'Criminology', 'Foreign Language (choose one)', 'HUMINT'], 'Extensive NCTC intelligence holdings and badges to major partner headquarters.', ['The OCR includes this profile just before the formal suggested-professions section, and it is too profession-specific to ignore.']),
  ...
  seed('Biological Resources Division', 'Scientist', 'National Park Service', 'Scientists and ecologists who preserve park ecosystems and advise on practical methods of protecting natural and historical resources.', ['History', 'Law', 'Search', 'Survival'], 'Scientific field gear matched to specialty plus camping and backcountry equipment.', ['The OCR presents BRD as both scientific and policy-adjacent.']),
  seed('Wildland Fire Division', 'Firefighter', 'National Park Service', 'NPS fire-management personnel ranging from hand crews to elite hotshots, fighting some of the largest and ugliest fires in the world.', ['Athletics', 'First Aid', 'Survival', 'Swim'], 'Rugged wildfire PPE, tools, pumps, shelters, ropes, radios, and personal medical kits.', ['The OCR strongly emphasizes the brutal environment, long deployments, and near-military teamwork of the role.']),
  seed('Interpretive Ranger', 'Anthropologist or Historian', 'National Park Service', 'Public-facing rangers who educate visitors while also protecting sites, history, and the people moving through them.', ['Firearms', 'First Aid', 'Medicine', 'Survival'], 'Uniform, badge, camping tools, and a practical library on site history, ecology, and survival.', ['The OCR includes a bespoke skill package blending history, HUMINT, persuasion, science, and field competence.'], {
    bonds: 2,
  }),
  ...
  seed('Flight Research, Test, and Engineering Directorate', 'Scientist', 'NASA Armstrong Flight Research Center', 'Aeronautical engineers and systems professionals who build and test experimental and next-generation flight systems.', ['Bureaucracy', 'Craft (Engineering)', 'Craft (Mechanic)', 'Science (Aeronautics)'], 'Engineering trade tools and direct access to aeronautical research programs.', ['The OCR names this role in-line rather than as a separate heading, but it is clearly intended as a profession profile.']),
  seed('Astronaut Corps Pilot', 'Pilot or Sailor', 'NASA', 'An elite astronaut-pilot profile built for spacecraft, remote systems, and high-stakes mission execution where failure is spectacular and unforgiving.', ['Heavy Machinery', 'Science (Biology)', 'Science (Mathematics)', 'Science (Meteorology)'], 'Hardened tools, launch-site and training-facility access, and mission-specific technical support.', ['The OCR gives this role a distinctive custom skill package focused on flight, navigation, physics, and spacecraft operation.'], {
    bonds: 2,
  }),
  ...
  seed('Microsystems Technology Office', 'Program Manager', 'DARPA', 'Program staff exploring practical applications for advanced microelectronics, exotic power sources, and discoveries adjacent to quantum-scale engineering.', ['Bureaucracy', 'Computer Science', 'Science (Mathematics)', 'Science (Physics)'], 'Program budget control and one fragile concept machine or prototype.', ['The OCR compresses this entry, but the mission remains clear.']),
  ...
  seed('Drug Policy Researcher', 'Scientist', 'RAND Corporation', 'Researchers studying drugs and alcohol to inform policy with evidence rather than rhetoric.', ['Criminology', 'Law', 'Science (Mathematics)', 'Pharmacy'], 'Research libraries, policy data, and strong professional networks in both law-enforcement and pharmaceutical worlds.', ['A stray OCR equipment line likely belongs here, and it has been preserved in the dossier.']),
...
const buildLongDescription = (entry: ComplexProfessionSeed): string => {
  ...
  lines.push('- Repaired and normalized from the OCR dump in `delta-green-the-complex`.');
  lines.push('- Wording has been cleaned for professional readability without intentionally removing role detail.');
...
export function buildComplexProfessions(baseProfessions: Profession[]): Profession[] {
  ...
  return COMPLEX_PROFESSION_SEEDS.map((entry) => {
    ...
    return {
      ...
      source: 'The Complex (OCR repair import)',
